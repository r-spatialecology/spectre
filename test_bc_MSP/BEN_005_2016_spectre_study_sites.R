...
# =================================================
data_list <- readRDS("./data/BEN_2016_bird_obs.rds") 
...
# from lm (full model)
alpha_model_full <- readRDS("./data/BEN_richness_2016.rds")

# Beta-diversity / commonness
# ===========================

gdm_predictions <- readRDS("./data/BEN_beta_2016.rds") # predicted Sorensen estimates, from Gabriel
...
pred_alpha_obs_beta_target <- spectre:::generate_commonness_matrix_from_gdm(gdm_predictions = observed_sorensen, 
                                                                            alpha_list = alpha_model_full)

obs_alpha_pred_beta_target  <- spectre:::generate_commonness_matrix_from_gdm(gdm_predictions = gdm_predictions, 
                                                                             alpha_list = alpha_list)

(MAE_pred_alpha_obs_beta <- mean(abs(pred_alpha_obs_beta_target - observed_commonness), na.rm = TRUE))
(RCE_pred_alpha_obs_beta <- MAE_pred_alpha_obs_beta / mean_observed_commonness * 100)

(MAE_obs_alpha_pred_beta <- mean(abs(obs_alpha_pred_beta_target - observed_commonness), na.rm = TRUE))
(RCE_obs_alpha_pred_beta <- MAE_obs_alpha_pred_beta / mean_observed_commonness * 100)
...
sim_fun <- function(siminputrow, parameters, writeRDS, verbose)  
{
  ...
  res_min_conf <- spectre::run_optimization_min_conf_0(alpha_list = alpha_model_full, # alpha_list, alpha_model_full, alpha_model_poly
  ...
  r_1 <- tibble::tibble(replicate = replicate,
                        ...
                        MAE_pred_alpha_obs_beta = MAE_pred_alpha_obs_beta,
                        RCE_pred_alpha_obs_beta = RCE_pred_alpha_obs_beta,
                        MAE_obs_alpha_pred_beta = MAE_obs_alpha_pred_beta,
                        RCE_obs_alpha_pred_beta = RCE_obs_alpha_pred_beta,
                        MAE_sol_obs = MAE_sol_obs,
                        RCE_sol_obs = RCE_sol_obs,
                        MAE_sol_pred = MAE_sol_pred, 
                        RCE_sol_pred = RCE_sol_pred)
  saveRDS(r_1, file = paste0("./results/BEN_2016_36sites/comm", siminputrow, ".rds")) 
}
