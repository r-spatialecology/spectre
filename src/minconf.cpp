...
// MSP start
int MinConf::optimize0_sorensen(const long max_steps_, const double max_energy, long long seed, const unsigned patience)
{
    ...
    auto energy = calc_energy_sorensen(sorensen, target_sorensen);
    ...
    while(energy > max_energy) {
        ...
        add_species_min_conf_sorensen(site, target_sorensen);
        const auto sorensen = calculate_sorensen();
        energy = calc_energy_sorensen(sorensen, target_sorensen);
    ...
    energy = calc_energy_sorensen(calculate_sorensen(solution), target_sorensen);
    iteration_count.push_back(iteration_count.back() + 1);
    energy_vector.push_back(energy);
    
    if (calc_energy_sorensen(calculate_sorensen(current_best_solution), target_sorensen) <
        energy) {
...
double MinConf::calc_energy_random_solution_sorensen(const unsigned n)
{
    double avg_energy = 0;
    for (unsigned i = 0; i < n; i++) {
        const auto random_solution = gen_random_solution();
        const auto sorensen = calculate_sorensen(random_solution);
        avg_energy += calc_energy_sorensen(sorensen, target_sorensen); // MSP
    }
...
// MSP start 
void MinConf::add_species_min_conf_sorensen(unsigned site,
...
// MSP start
std::vector<unsigned> MinConf::calc_min_conflict_species_sorensen(const unsigned site,
                                                                  ...
                                                                  const std::vector<std::vector<double> > &target_sorensen)
{
    ...
    for (unsigned species_idx = 0; species_idx < free_species.size(); species_idx++) {
        ...
        double energy_ = calc_energy_sorensen(sorensen_new, target_sorensen, true); // may use a different matrix norm
